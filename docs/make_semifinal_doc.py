...
table(doc, [
    ...
    ["文檔日期", "2026 年 8 月 27 日"],
], widths=[1.75, 5.05], header=False)

callout(doc, "評審 30 秒入口",
        "打開公網 → 按「90 秒評審快速演示」→ 行程內領一次性到店碼 → 儀表板輸入商戶 PIN 2580 核銷兩次（第二次被拒）。"
        "以下截圖均取自該公網實例，非設計稿。")
...
table(doc, [
    ["複賽官方要求", "本作品對應", "核驗方式"],
    ["完成部署可實際使用的作品",
     "公網 24 小時運行：單日自訂問題由真實 qwen3.7-plus 推理；多日/評審模式以可重現工具鏈完成；五語言、地圖時間軸與 PDF 匯出",
     f"直接使用 {SITE}（無需登入）"],
    ["呈現效果須達到計劃書的指標",
...
bullet(doc, "回歸測試擴充至 647 後端、119 瀏覽器、102 API/安全，全部 PASS；8 個 MCP 工具協議測試通過。")

h2(doc, "公網實拍（2026-08-27）")
...
bullet(doc, "647 後端、119 瀏覽器、102 API/安全、8 MCP 協議測試全部 PASS；另有倉庫交付一致性測試（qa/ 目錄可重現）。", head="質量：")
